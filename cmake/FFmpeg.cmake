...
# Build an expanded search list that also covers multiarch subdirectories,
# e.g. /usr/include -> also try /usr/include/x86_64-linux-gnu (Debian/Ubuntu multiarch layout)
set(_ffmpeg_inc_search_dirs ${FFMPEG_INCLUDE_DIRS})
foreach(_inc_dir ${FFMPEG_INCLUDE_DIRS})
    foreach(_multiarch x86_64-linux-gnu aarch64-linux-gnu arm-linux-gnueabihf)
        if(EXISTS "${_inc_dir}/${_multiarch}")
            list(APPEND _ffmpeg_inc_search_dirs "${_inc_dir}/${_multiarch}")
        endif()
    endforeach()
endforeach()

set(FFMPEG_CHECK_FILE "")
foreach(_inc_dir ${_ffmpeg_inc_search_dirs})
    if(EXISTS "${_inc_dir}/libavformat/avformat.h")
        set(FFMPEG_CHECK_FILE "${_inc_dir}/libavformat/avformat.h")
        # Make sure this dir (may be a newly found multiarch subdir) is in FFMPEG_INCLUDE_DIRS
        list(APPEND FFMPEG_INCLUDE_DIRS "${_inc_dir}")
